...
//Minimal drawer (Draws only the chamber)
	...
	public void scene(PGraphics3D p3d) {	
		...
		for(BSimControlledBacterium b : bacteria) { //For each bacterium here
			...
			if(rateTL<=0.5) {
				...
				if(rateTL<thres) {
					green=255-(int) Math.floor(((rateTL/thres)*255)); //I set the color in a way taht it is red if it has not writched and green otherwise	
					red=(int) Math.floor(((rateTL/thres)*255));
				}
				else {
					red=255;
				}		
